...
export async function ensureSchema() {
  ...
  await q`ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS cancel_reason TEXT DEFAULT ''`;
  await q`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS size_material TEXT DEFAULT ''`;
  await q`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS work_file_link TEXT DEFAULT ''`;
  await q`ALTER TABLE jobs ADD COLUMN IF NOT EXISTS product_type TEXT DEFAULT ''`;
  await q`ALTER TABLE enquiries ADD COLUMN IF NOT EXISTS product_category TEXT DEFAULT ''`;
...
// Auto-cancel enquiries that have had no changes (no edits) in 30 days.
// Runs on every enquiries load — self-healing, no separate cron needed.
// "Confirmed" and already-"Cancelled" enquiries are left alone.
export async function autoCancelStaleEnquiries(q) {
  try {
    const { entry, parseHistory } = await import("./history");
    const stale = await q`SELECT enquiry_id, history FROM enquiries
      WHERE status NOT IN ('Cancelled', 'Confirmed')
      AND updated_at < now() - interval '30 days'`;
    for (const row of stale) {
      const history = parseHistory(row.history);
      history.push(entry("system", "Enquiry auto-cancelled — no changes made in 30 days"));
      await q`UPDATE enquiries SET status='Cancelled', cancel_reason='Auto-cancelled after 30 days with no changes', history=${JSON.stringify(history)}, updated_at=now() WHERE enquiry_id=${row.enquiry_id}`;
    }
  } catch {
    // Non-fatal — the list still loads even if the sweep fails
  }
}
