...
const HEADERS = ["Job ID", "Order Date", "Customer", "Mobile", "Product", "Quantity", "Work Type", "Machine Type", "Designer", "Design Required", "Design Status", "Production Status", "Delivery Status", "Order Status", "Priority", "Delivery Date", "Payment", "Google Review", "Cancel Reason", "Production Complete", "Last Updated", "Notes"];
...
const ENQ_HEADERS = ["Enquiry ID", "Date", "Customer", "Mobile", "Product Type", "Size / Material", "Quantity", "Design Required", "Priority", "Enquiry Mode", "System", "Status", "Last Updated"];
...
export async function syncEnquiriesToSheet(q) {
  if (!sheetConfigured()) return;
  try {
    const enquiries = await q`SELECT enquiry_id, customer_name, mobile, product_type, size_material, quantity, design_required, priority, enquiry_mode, designer_name, status, created_at, updated_at FROM enquiries ORDER BY created_at DESC`;
