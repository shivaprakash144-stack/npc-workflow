...
const HEADERS = ["Job ID", "Order Date", "Customer", "Mobile", "Product Category", "Product Type", "Size", "Quantity", "Work Type", "Machine Type", "Designer", "Design Required", "Design Status", "Working File Link", "Production Status", "Delivery Status", "Order Status", "Priority", "Delivery Date", "Payment", "Google Review", "Cancel Reason", "Production Complete", "Last Updated", "Notes"];
...
export async function syncJobsToSheet(q) {
  ...
  try {
    ...
    const rows = jobs.map((j) => [
      ...
      j.product_category, j.product_type, j.size_material, j.quantity, j.work_type, j.machine_type, j.designer_name,
...
const ENQ_HEADERS = ["Enquiry ID", "Date", "Customer", "Mobile", "Product Category", "Product Type", "Size / Material", "Quantity", "Design Required", "Priority", "Enquiry Mode", "System", "Status", "Cancel Reason", "Last Updated"];
...
export async function syncEnquiriesToSheet(q) {
  if (!sheetConfigured()) return;
  try {
    const enquiries = await q`SELECT enquiry_id, customer_name, mobile, product_category, product_type, size_material, quantity, design_required, priority, enquiry_mode, designer_name, status, cancel_reason, created_at, updated_at FROM enquiries ORDER BY created_at DESC`;
    const rows = enquiries.map((e) => [
      e.enquiry_id,
      e.created_at ? new Date(e.created_at).toLocaleDateString("en-IN", { timeZone: "Asia/Kolkata" }) : "",
      e.customer_name, e.mobile, e.product_category, e.product_type, e.size_material, e.quantity,
