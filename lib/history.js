...
export function jobChanges(oldJ, b, derivedStatus) {
  ...
  const track = [
    ...
    ["work_file_link", "Working file location"],
  ];
...
// Compare old enquiry row vs incoming update and describe every change.
export function enquiryChanges(oldE, b) {
  const ch = [];
  const track = [
    ...
    ["design_required", "Design required"],
    ["priority", "Priority"],
    ["designer_name", "System"],
    ["enquiry_mode", "Enquiry mode"],
    ["status", "Status"],
  ];
  for (const [k, label] of track) {
    const o = String(oldE[k] ?? "");
    const n = String(b[k] ?? "");
    if (o !== n) ch.push(`${label}: ${o || "—"} → ${n || "—"}`);
  }
  return ch;
