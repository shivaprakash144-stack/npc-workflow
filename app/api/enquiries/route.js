...
export async function POST(req) {
  ...
  try {
    ...
    await q`INSERT INTO enquiries (enquiry_id, customer_id, customer_name, mobile, product_type, size_material, quantity, design_required, ref_image, est_price, status, history)
      VALUES (${id}, ${b.customer_id || ""}, ${b.customer_name.trim()}, ${String(b.mobile).trim()}, ${b.product_type}, ${b.size_material || ""}, ${b.quantity}, ${b.design_required || "No"}, ${b.ref_image || ""}, ${b.est_price || ""}, ${b.status || "New Enquiry"}, ${history})`;
...
export async function PUT(req) {
  ...
  try {
    ...
    await q`UPDATE enquiries SET customer_name=${b.customer_name.trim()}, mobile=${String(b.mobile).trim()}, product_type=${b.product_type}, size_material=${b.size_material || ""}, quantity=${b.quantity}, design_required=${b.design_required || "No"}, ref_image=${b.ref_image || ""}, est_price=${b.est_price || ""}, status=${b.status || "New Enquiry"}, history=${JSON.stringify(history)}, updated_at=now() WHERE enquiry_id=${b.enquiry_id}`;
